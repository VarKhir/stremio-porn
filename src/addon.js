...
builder.defineCatalogHandler(async ({ type, id, extra, config }) => {
  try {
    ...
    let sortProp =
      `${PornClient.SORT_PROP_PREFIX}${adapterName}`
    ...
    let methodName =
      extra.search ? 'meta.search' : 'meta.find'
    let results =
      await client.invokeMethod(methodName, request)
...
builder.defineMetaHandler(async ({ type, id, config }) => {
  try {
    ...
    let result =
      await client.invokeMethod('meta.get', request)
...
builder.defineStreamHandler(async ({ type, id, config }) => {
  try {
    ...
    let results =
      await client.invokeMethod('stream.find', request)
